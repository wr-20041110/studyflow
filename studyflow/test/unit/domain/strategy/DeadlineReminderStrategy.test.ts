...
describe('DeadlineReminderStrategy', () => {
  ...
  describe('shouldTrigger', () => {
    ...
    it('当任务恰好在指定天数前到期时应触发提醒', () => {
      ...
      const currentTime = new Date('2026-06-09T00:00:00Z'); // 1天后到期
    ...
    it('当任务在2天后到期时应触发2天前提醒策略', () => {
      ...
      const currentTime = new Date('2026-06-08T00:00:00Z'); // 2天后到期
    ...
    it('当任务已过期时不应触发提醒', () => {
      const strategy = new DeadlineReminderStrategy(1);
      const currentTime = new Date('2026-06-11T00:00:00Z'); // 已过期
    ...
    describe('边界值测试', () => {
      it('应该在到期当天（0天）不触发提醒', () => {
        const strategy = new DeadlineReminderStrategy(1);
        const currentTime = new Date('2026-06-10T00:00:00Z'); // 到期当天
      ...
      it('应该正确处理小时级别的差异', () => {
        const strategy = new DeadlineReminderStrategy(1);
        const currentTime = new Date('2026-06-08T12:00:00Z'); // 1.5天后到期
  ...
  describe('generateMessage', () => {
    it('应该生成包含任务标题和到期日期的提醒消息', () => {
      ...
      expect(message).toContain('2026/6/10');
    });
