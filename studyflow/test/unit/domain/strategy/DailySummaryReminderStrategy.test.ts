...
describe('DailySummaryReminderStrategy', () => {
  ...
  describe('shouldTrigger', () => {
    ...
    beforeEach(() => {
      task = Task.create('task-1', '测试任务', '测试描述', Priority.MEDIUM, new Date('2026-06-17'), 'user-1');
    });

    it('在指定汇总时间应对未完成任务触发提醒', () => {
      const strategy = new DailySummaryReminderStrategy({ hour: 9, minute: 0 });
      const currentTime = new Date('2026-06-16T09:00:00');
    ...
    it('在指定汇总时间应对进行中的任务触发提醒', () => {
      ...
      const currentTime = new Date('2026-06-16T18:00:00');
    ...
    it('在非汇总时间不应触发提醒', () => {
      const strategy = new DailySummaryReminderStrategy({ hour: 9, minute: 0 });
      const currentTime = new Date('2026-06-16T08:59:00');
    ...
    it('应该正确处理不同日期的同一时间', () => {
      const strategy = new DailySummaryReminderStrategy({ hour: 9, minute: 0 });
      const time1 = new Date('2026-06-16T09:00:00');
      const time2 = new Date('2026-06-17T09:00:00');
    ...
    describe('边界值测试', () => {
      it('应该正确处理汇总时间的前一分钟', () => {
        const strategy = new DailySummaryReminderStrategy({ hour: 9, minute: 0 });
        const currentTime = new Date('2026-06-15T09:00:00');
      ...
      it('应该正确处理边界时间点（23:59）', () => {
        ...
        const currentTime = new Date('2026-06-15T23:59:00');
      ...
      it('应该正确处理边界时间点（00:00）', () => {
        ...
        const currentTime = new Date('2026-06-16T00:00:00');
  ...
  describe('generateMessage', () => {
    it('应该生成包含任务标题的汇总消息', () => {
      const strategy = new DailySummaryReminderStrategy();
      const task = Task.create('task-1', '完成 TypeScript 课程', '学习 TypeScript', Priority.MEDIUM, new Date('2026-06-15'), 'user-1');
    ...
    it('消息应包含每日汇总标识', () => {
      const strategy = new DailySummaryReminderStrategy();
      const task = Task.create('task-1', '测试任务', '测试', Priority.MEDIUM, new Date('2026-06-15'), 'user-1');
