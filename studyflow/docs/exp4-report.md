...
围绕代码从开发到集成的全生命周期，建立了**三层质量门禁**：

| 层级 | 阶段 | 工具 | 耗时 | 拦截目标 |
|------|------|------|------|----------|
| 一 | 静态检查 | ESLint | 秒级 | 语法错误、代码风格、未使用变量、潜在逻辑问题 |
| 二 | 单元测试 | Jest (190 tests) | 分钟级 | 业务逻辑正确性、回归检测、边界条件覆盖 |
| 三 | 编译检查 | TypeScript (`tsc`) | 秒级 | 类型安全、导入路径、接口契约完整性 |

三层按**成本递增**排序：lint 在最前（廉价），test 在中间（昂贵），build 在最后（验证可交付物）。问题在最早、最便宜的阶段被拦截，避免在昂贵阶段才发现低级错误。

### 本地质量保障（提交前）

```bash
npm run lint   # 第一道关：静态分析
npm test        # 第二道关：190 个测试用例
npm run build   # 第三道关：确认可编译为 JS
```
...
| 机制 | 说明 | 为什么重要 |
|------|------|------------|
| **触发全覆盖** | `push` + `pull_request` 双触发 | 每次推送和合并请求都必须通过质量门禁，杜绝绕过 |
| **依赖精确锁定** | `npm ci` 而非 `npm install` | 严格按 `package-lock.json` 安装，确保 CI 环境与本地、他人环境完全一致 |
| **缓存加速** | `cache: 'npm'` | 避免每次下载全部依赖，将 CI 耗时从分钟级压缩至秒级 |
| **版本矩阵** | `matrix.node-version: [20.x]` | 预留多版本扩展能力，后续可增加 18.x、22.x 并行验证 |
| **分层顺序** | lint → test → build | 廉价检查前置，避免测试跑完才发现类型错误 |
| **独立环境** | `ubuntu-latest` 全新实例 | 每次 CI 从零构建，杜绝"在我机器上能跑"的环境依赖问题 |
...
git push → GitHub Actions 自动触发
              │
              ├─ (1) npm ci       → 依赖安装失败？→ ❌ 阻塞
              ├─ (2) npm run lint → 静态检查不过？→ ❌ 阻塞
              ├─ (3) npm test     → 190 测试有失败？→ ❌ 阻塞
              ├─ (4) npm run build → 编译失败？   → ❌ 阻塞
              │
              └─ ✅ 全部通过 → 允许合并至 master
```

任何一步失败都会阻止代码进入主分支，确保 `master` 分支始终处于可交付状态。
...
| 维度 | 结果 | 关键发现 |
|------|------|----------|
| 1. 是否破坏原有契约 | ✅ 兼容 | `tags` 参数默认 `[]`，`Task.create()` 新增可选参数，旧代码零影响 |
| 2. 是否引入重复逻辑 | ⚠️ 轻微 | `toDto()` 在 2 个 UseCase 中重复，暂不提取（避免过度工程化） |
| 3. 边界遗漏检查 | ✅ 全面 | 空名称、超长名、空数组、无效颜色、已完成任务保护等 9 项全部覆盖 |
| 4. 测试与改动匹配 | ✅ 充分 | 新增 32 个测试，覆盖正常路径、边界值、错误路径和业务规则 |
| 5. 提交说明动机 | ✅ 清晰 | 类型正确、标题简洁、动机明确 |
...
|------|------|------|------|
| 🔴 严重 | 0 | — | — |
| 🟡 一般 | 2 | `toDto()` 代码重复、CLI 动态导入风格 | `toDto()` 暂不处理（仅 2 处），CLI 动态导入已改为静态 |
| 🔵 建议 | 2 | 颜色可配置化、AND 筛选语义 | 列入后续迭代计划 |
...
| 空标签数组筛选 | 返回空数组 | ✅ |
| 不存在的标签查询 | 返回空结果 | ✅ |
| 已完成任务加标签 | 抛 `Cannot add tag to a completed task` | ✅ |
| 重复添加同名标签 | 幂等，静默忽略 | ✅ |
| 移除不存在的标签 | 抛 `Tag not found` | ✅ |
| 多用户标签隔离 | `userId` 过滤 | ✅ |

### 审查总结

本次变更质量良好，无严重问题。标签功能设计合理，向后兼容，测试覆盖充分。AI 辅助审查在快速扫描大量变更、系统性检查边界覆盖方面体现价值，但业务语义判断（如空标签返回空结果）、架构影响评估（DDD 分层是否被削弱）、重构时机把握（何时提取重复代码）仍需人工决策。
